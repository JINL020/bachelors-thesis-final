...
import com.minimal.ec135.MyNotificationService
...
class DetectionsProcessor(
    ...
    fun processDetectionResults(myDetections: List<MyDetection>, viewModel: MainViewModel) {
        ...
        myDetections.forEach { detection ->
            ...
            ocrTask.addOnSuccessListener { result ->
                ...
                val ocrScore = findSimilarity(searchNumber, ocrText)
                ...
                if (shouldAdd && (ocrScore >= ocrThreshold)) {
                    ...
                    service.showNotification(searchNumber)
                }
            }
        }
    }

    private fun getOcrText(textBlocks: MutableList<Text.TextBlock>): String {
        try {
    ...
    private fun findSimilarity(x: String, y: String): Float {
